...
class PostgresDatabaseImplTest {
    ...
    public static void beforeAll() throws Exception {
        cosmos = new CosmosBuilder().withDatabaseType("postgres")
                ...
                .withConnectionString(EnvUtil.getOrDefault("POSTGRES_CONNECTION_STRING", ""))
                .build();
        // we do not need to create a collection here, so the second param is empty
        // we create collections in specific test cases
        db = (PostgresDatabaseImpl) cosmos.createIfNotExist(host, "");
        db.createTableIfNotExists("Users");
        db.createTableIfNotExists("Users2");
        db.createTableIfNotExists("Families");
        db.createTableIfNotExists("SheetContents");
        db.createTableIfNotExists("PatchTests");
        db.createTableIfNotExists("PatchPojoTests");
        db.createTableIfNotExists("IncrementTests");
        db.createTableIfNotExists("NumberTest");
    ...
    void addEtag4_should_work() {
        ...
        { // expireAt should not be added if expireAtEnabled is false
            ...
            var cosmosExpireOff = new CosmosBuilder().withDatabaseType("mongodb")
                    ...
                    .withConnectionString(EnvUtil.getOrDefault("MONGODB_CONNECTION_STRING", PostgresImplTest.LOCAL_CONNECTION_STRING))
                    .build();
