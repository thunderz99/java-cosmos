...
class MongoDatabaseImplTest {
    ...
    void update_should_work_for_empty_key() throws Exception {
        ...
        try {

            {
                ...
                var map4Update = Map.of("id", id1, "name", "Jane", "", "emptyValue2");
                var patched = db.update(host, map4Update, "Users").toMap();
...

            {
                ...
                assertThat(upserted.get("id")).isEqualTo(id1);
                assertThat(upserted.get("name")).isEqualTo("first01");
                assertThat((Map<String, Object>) upserted.get("sheet-2")).containsEntry("", "emptyValue");


                // update partial should work for empty key
                var map4Update = Map.of("id", id1, "name", "John", "sheet-2", Map.of("", "emptyValue2"));
                var patched = db.update(host, map4Update, "Users").toMap();
    ...
    void updatePartial_should_replace_nested_empty_map_when_enabled() throws Exception {
        ...
        try {
            var original = db.upsert(host, data, partition).toMap();
            ...
            var updated = db.updatePartial(host, id, partialData, partition, option).toMap();
            ...
            assertThatThrownBy(() -> db.updatePartial(host, id, partialData, partition, option))
    ...
    @Test
    void updatePartial_should_replace_nested_empty_map_in_empty_key_fallback() throws Exception {
        var partition = "SheetContents";
        var id = "replace_nested_empty_map_fallback_" + RandomStringUtils.randomAlphanumeric(6);
        var data = Map.of(
                "id", id,
                "values", Map.of(
                        "rules", Map.of("source", "target"),
        ...
        try {
            ...
            var partialData = new LinkedHashMap<String, Object>();
            partialData.put("values", Map.of("rules", Map.of()));
            partialData.put("", "empty-key-value");

            var updated = db.updatePartial(host, id, partialData, partition,
                    new PartialUpdateOption().replaceEmptyMap(true)).toMap();
            var values = (Map<String, Object>) updated.get("values");

            assertThat((Map<String, Object>) values.get("rules")).isEmpty();
            assertThat(values).containsEntry("sibling", "keep");
            assertThat(updated).containsEntry("", "empty-key-value");
        } finally {
            db.delete(host, id, partition);
        }
    }
    ...
    void find_should_work_with_filter_comparing_null() throws Exception {
        ...
        try {
            ...
            // id OR accountId = null, this grammar should be supported
            {
    ...
    void batchPatch_should_patch_multiple_documents() throws Exception {
        BatchPatchContractTestSupport.assertMultipleDocumentsArePatched(db, host, "Users");
    }

    @Test
    void batchPatch_should_apply_different_operations_per_document() throws Exception {
        BatchPatchContractTestSupport.assertDifferentOperationsAreApplied(db, host, "Users");
    }

    @Test
    void batchPatch_should_rollback_all_updates_when_one_document_is_missing() throws Exception {
        BatchPatchContractTestSupport.assertAllUpdatesAreRolledBack(db, host, "Users");
    }

    @Test
    void batchPatch_should_reject_more_than_one_hundred_documents() {
        BatchPatchContractTestSupport.assertMoreThanOneHundredDocumentsAreRejected(db, host, "Users");
    }

    @Test
    void batchPatch_should_reject_more_than_ten_operations_per_document() {
        BatchPatchContractTestSupport.assertMoreThanTenPatchOperationsAreRejected(db, host, "Users");
    }
